...
public class JsfLoginUrlAuthenticationEntryPoint implements AuthenticationEntryPoint, InitializingBean {
	...
	/**
	 * Set to true to force login form access to be via https. If this value is true
	 * (the default is false), and the incoming request for the protected resource
	 * which triggered the interceptor was not already <code>https</code>, then the
	 * client will first be redirected to an https URL, even if
	 ...
	 *
	 * @param useForward true if a forward to the login page should be used. Must be
